...
constexpr size_t kCacheCapacity{20 * 1024 * 1024};
...
StringCache::StringCache()
    : m_arena_begin{[]() {
        ...
        void* arena{};
        int const error = posix_memalign(&arena, static_cast<size_t>(page_size),
                                         kCacheCapacity);
...
StringCache::~StringCache() {
  std::cerr << "StringCache[" << static_cast<void*>(this) << "] "
            ...
            << (m_arena_cursor - m_arena_begin) << " bytes, " << m_hits
            << " hits.\n";
